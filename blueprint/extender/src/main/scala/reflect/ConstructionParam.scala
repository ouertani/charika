...
case class ConstructionParam( dependsOn:List[String], arguments : List[Argument],  propertys :  List[Property]) {
  need(dependsOn != null, "DependsOn must not be null!")
  need(arguments != null, "arguments must not be null!")
  need(propertys != null, "propertys must not be null!")
  ...
  if( t._1.size !=0) {
     ...
     *
     */
    need (t._2.size==0,"Either all arguments have a specified index or none have a specified index.")
    ...
    need(l.filter(0>).isEmpty,"index must be >= 0")

    l.sortWith  (_ < _).foldLeft(-1) ((x,y)=> { need(y==x+1,"index run from 0..(n-1), where n is the number of arguments");y})
